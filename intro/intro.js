...
// Define the loan terms in months
var loanTerms = Array.from({ length: 120 - 6 + 1 }, (_, i) => 120 - i);
...
// Primary variables
var rt = 0.2; // Annual interest rate for 20%
...
monthlyPayDiv.addEventListener("input", function () {
  ...
  if (number < 134.59 || number > totalMonthlyAmountDisplay(loanMax)) {
...
function totalMonthlyAmountDisplay(selectedLoanAmount) {
  ...
  if (
    selectedLoanAmount.toString() === "undefined" ||
    selectedLoanAmount.toString() === "NaN"
  ) {
    return null;
  }
...
function viewLoanDetailsDiv() {
  ...
  apr =
    calculateRate(loanTerm, -totalMonthlyInstalment, loanAmount, 0, 0, 0.1) *
    1200;

  document.getElementById("InterestRate").textContent = Number(
    interest.toFixed(0)
  );
  document.getElementById("LoanAmountRequested").textContent =
    loanAmount + ".00";
  ...
  document.getElementById("MonthlyInstalments").textContent = Number(
    monthlyLoanInstalment.toFixed(2)
  );
  document.getElementById("MonthlyCreditLifePremium").textContent =
    monthlyInsurancePremiumm;
  document.getElementById("MonthlyCollectionFee").textContent =
    monthlyEmployerCollection;
  document.getElementById("TotalMonthlyInstalments").textContent =
    ...
    const df =
      ...
      pmt *
        ...
        ((nper * Math.pow(1 + rate, nper - 1)) / rate -
          (Math.pow(1 + rate, nper) - 1) / Math.pow(rate, 2));
...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  function updateLoanInterest() {
    ...
    switch (
      loanInterestDropdown.options[loanInterestDropdown.selectedIndex]
        .textContent
    ) {
      case "botusafe 23%":
        c9 = 1 - 0.0271;
  ...
  function decrement() {
    ...
    if (value > loanMax) {
      ...
      // Adjust the min value as needed
      value -= 500;
    } else if (value < 5000) {
      return;
    }
...
document.addEventListener("click", function (event) {
  ...
  if (
    ...
    event.target.id !== "viewLoanDetails"
  ) {
